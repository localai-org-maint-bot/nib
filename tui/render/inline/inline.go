...
// nothing for a Presenter to render differently. DialogApproval lays out
// Rows (the argument card, or — when RowsUnstructured — a single wrapped
// prose block), Hint (the captured reasoning, wrapped) and Options (the
// choice menu, one line per option styled per its Emphasis).
